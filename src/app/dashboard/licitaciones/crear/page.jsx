...
const CrearLicitacionPage = () => {
  ...
  const handleFormatoChange = async (value) => {
    ...
    // Cargar procesos del formato desde BD
    if (formatoId) {
  ...
  const handleSubmit = async (values) => {
    ...
    const formatoSeleccionado = formatos.find(f => f.id === selectedFormato)
    ...
    if (result.success) {
      ...
      message.error(result.error || "Error al crear el proceso")
    }
  }
          ...
          <Card className={styles.card}>
            ...
            >
              ...
              <Form.Item
                ...
                rules={[{ required: true, message: "Seleccione un producto/servicio" }]}
              >
                <Select
            ...
            <Col xs={24} className={styles.rightPanelCol}>
              {selectedFormato && formatoSeleccionado ? (
                ...
                >
                  {loadingProcesos ? (
                  ...
                  ) : procesosFormato.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "20px" }}>
                      <Empty 
                        image={Empty.PRESENTED_IMAGE_SIMPLE}
                        description="Este formato no tiene procesos configurados"
                      />
                    </div>
                  ) : (
                    <div className={styles.workflowContainer}>
                      {getProcesosVisibles(procesosFormato, {
                        ...
                        return (
                          <div key={pasoObj.numero || pasoObj.numeroPaso} className={styles.workflowStepWrapper}>
                            <div 
                              className={`${styles.workflowStepRow} ${isExpanded ? styles.workflowStepRowOpen : ''} ${isSubpaso ? styles.workflowSubstepRow : ''}`}
                              onClick={() => hasSubpasos && toggleStepExpansion(pasoObj.numero)}
                            >
                              <div className={styles.workflowStepMain}>
                                <span className={`${styles.workflowStepNumber} ${isExpanded ? styles.workflowStepNumberOpen : ''} ${isSubpaso ? styles.workflowSubstepNumber : ''}`}>
                                  {pasoObj.numero}
                                </span>
