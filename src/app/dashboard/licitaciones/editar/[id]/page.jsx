...
import { getFormatosLiquidacion, getRequirentes, getLicitacionById, updateLicitacion } from "@/actions/licitaciones"
...
const EditarLicitacionPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [formatosRes, requirentesRes, licitacionRes] = await Promise.all([
      ...
      if (licitacionRes.data) {
        const lic = licitacionRes.data
        setLicitacion(lic)
        setSelectedFormato(lic.formatoLiquidacionId)

        form.setFieldsValue({
          formatoLiquidacion: lic.formatoLiquidacionId,
          requirente: lic.requirente,
          nombreLicitacion: lic.nombreLicitacion,
          numeroLicitacion: lic.numeroLicitacion,
          vigencia: lic.vigencia ? dayjs(lic.vigencia) : null,
          montoPresupuestado: lic.montoPresupuestado ? parseInt(lic.montoPresupuestado.replace(/[^0-9]/g, "")) : null
        })
      }
    }
  ...
  const handleSubmit = async (values) => {
    ...
    if (result.success) {
      message.success("Licitación actualizada correctamente")
      router.push("/dashboard/licitaciones/todas")
    } else {
      message.error(result.error || "Error al actualizar la licitación")
    }
  }
    ...
    <div className={styles.container}>
      <Row gutter={24}>
        <Col xs={24} lg={14}>
          <Card className={styles.card}>
            <Title level={3} style={{ marginBottom: 24 }}>Editar Licitación</Title>
            ...
            >
              ...
              <Form.Item style={{ marginTop: 24 }}>
                <Space>
                  ...
                  <Button onClick={() => router.push("/dashboard/licitaciones/todas")}>
                    Cancelar
                  </Button>
                </Space>
              </Form.Item>
      ...
      >
        <div className={styles.loadingModal}>
          ...
          <Text>Actualizando licitación...</Text>
        </div>
      </Modal>
    </div>
  )
...
export default EditarLicitacionPage
